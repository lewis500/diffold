...
function quadFunLeft(quadTools) {
  // =====setup=====
  var M = {
    top: 20,
    right: 10,
  ...
  function link(scope, el, attr) {
    ...
    var svg = .append("g").translate([M.left, M.top]);
...
function quadFunMiddle() {
  ...
  var y = d3.scale.linear().domain([-2, 10]);
  var y2 = d3.scale.linear().domain([-2, 10]).clamp(true);
